...
void IOmanager(string trace_file_path){
    ...
    while (getline(traceFile, line)) {
        ...
        unsigned long long page_number=actual_address>>10; //for page size=10kb
        cout<<task<<" "<<address<<" "<<size<<":";
        if(string_keys_checker(program_to_page_table,task)==true){
            if(program_to_page_table[task][page_number]!=0){ //page table hit
                ...
                program_to_page_table[task][page_number]=frames_allocator(free_frame__numbers_address,allocated_frame_numbers_address,memory_to_allocate,PAGE_SIZE,frame_offset);
                ...
                std::stringstream ss;
                ss<<hex <<program_to_page_table[task][page_number];
