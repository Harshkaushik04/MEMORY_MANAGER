...
public:
    ...
    void activate(unsigned long long second_level_pt_size){
        unsigned long long* second_level_pt=new unsigned long long[second_level_pt_size];
        this->next=second_level_pt;
    }
};

class SpecialLinkedList{
public:
    Node* first_level_pt; //array of nodes
    SpecialLinkedList(){
        this->first_level_pt=nullptr;
    }
    SpecialLinkedList(Node* first_level_pt){
        this->first_level_pt=first_level_pt;
    }
};
bool string_keys_checker_3(unordered_map<string,SpecialLinkedList> map,string to_check){
    bool flag=false;
    for (pair<string,SpecialLinkedList> pair : map) {
        if(pair.first==to_check){
            flag=true;
            return flag;
        }
...
void IOmanager_double_level_page_table(string trace_file_path,int verbose){
    ...
    unordered_map<string,SpecialLinkedList> program_to_first_level_page_table; //mapping from program to respective page table
    ifstream traceFile(trace_file_path);
    if (!traceFile) {
        std::cerr << "Error: Could not open the file!" << std::endl;
        return;
    ...
    while (getline(traceFile, line)) {
        ...
        if(string_keys_checker_3(program_to_first_level_page_table,new_task.task_id)==true){
            if(program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next!=nullptr){
                if(program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[page_number]!=0){ //page miss
                    num_page_table_hits+=1; 
                    if(verbose==1){
                        cout<<"Page tabel hit"<<endl;
                    }
                    ...
                    num_page_table_miss+=1;
                    total_memory_allocated+=memory_to_allocate;
                    program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[page_number]=frames_allocator_3(free_frame__numbers_address,allocated_frame_numbers_address,memory_to_allocate,PAGE_SIZE,frame_offset);
                    ...
                    std::stringstream ss;
                    ss<<hex <<program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[page_number];
                    string hex_string = ss.str();
                    if(verbose==1){
                        cout<<hex_string<<endl;
                    }
                ...
                if((int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))%2==0){
                    program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next=new unsigned long long[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
                    for(long j=0;j<(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2);j++){
                        program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[j]=0;
                    }
                }
                else{
                    program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next=new unsigned long long[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
            ...
            num_page_table_miss+=1;
            if((int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))%2==0){
                Node* first_level_pt=new Node[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
                program_to_first_level_page_table[new_task.task_id]=SpecialLinkedList(first_level_pt);
                program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next=new unsigned long long[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
                for(long j=0;j<(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2);j++){
                        program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[j]=0;
                }
                // for(long i=0;i<(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2);i++){ //making 2D array
                //     program_to_first_level_page_table[new_task.task_id].first_level_pt[i].next=new unsigned long[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
                //     for(long j=0;j<(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2);j++){
                ...
                Node* first_level_pt=new Node[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2+1)];
                program_to_first_level_page_table[new_task.task_id]=SpecialLinkedList(first_level_pt);
                program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next=new unsigned long long[(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2)];
                for(long j=0;j<(int)pow(2,(int)(log2(VIRTUAL_MEMORY_SIZE)-log2(PAGE_SIZE))/2);j++){
                        program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[j]=0;
            ...
            memory_for_page_tables+=sizeof(unsigned long long)*VIRTUAL_MEMORY_SIZE/PAGE_SIZE;
            std::stringstream ss;
            ss<<hex <<program_to_first_level_page_table[new_task.task_id].first_level_pt[page_table_number].next[page_number];
            string hex_string = ss.str();
            if(verbose==1){
    ...
    clock_t end=clock();
    for(int i=0;i<memory_keys.end()-memory_keys.begin();i++){
        delete[] program_to_first_level_page_table[memory_keys[i]].first_level_pt;
    }
    cout<<"=============================================="<<endl;
    cout<<"count:"<<count<<endl;
    cout<<"Time taken:"<<double(end-start)/CLOCKS_PER_SEC<<" sec"<<endl;
